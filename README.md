...
- `jasyl/assets/screen-1…6.png` — скриншоты приложения
- `jasyl/assets/certificate.jpg` или `certificate.pdf` — сертификат Jasyl
- `jasyl/assets/photo-*.jpg` — фотографии с хакатона (уже на месте)

Ничего в коде править не нужно: положил файл — блок появился. Подходят `.png`,
`.jpg` и `.jpeg`.

**Важно про скриншоты:** страница сначала ищет только `screen-1`, и остальные
проверяет, лишь когда нашла первый, — иначе пустая секция каждый раз стучалась
бы по восемнадцати несуществующим адресам и засоряла консоль. Поэтому нумерацию
начинать с `screen-1`, а не с `screen-2`.

## Почему в vercel.json стоит trailingSlash

Страница лежит по адресу `/qa-dashboard/`, а внутри неё все пути относительные
(`assets/certificate.jpg`). Если открыть её без завершающего слэша, браузер
считает базой корень домена и просит `alikhandev.com/assets/certificate.jpg` —
получается 404 на всех картинках. `"trailingSlash": true` заставляет Vercel
приводить адрес к `/qa-dashboard/`, а в самих страницах на всякий случай стоит
короткий скрипт, который проставляет `<base>`, если слэша всё-таки нет.
